...
======================
Azure DevOps + GitHub integration and operations utility.

This script is used by automation agents and CI/CD pipelines to inspect,
manage, and synchronize Azure DevOps pipeline workflows with GitHub.
...
Environment variables for GitHub-related commands:
  GITHUB_PAT            - GitHub Personal Access Token
  GITHUB_REPO           - owner/repo format (for example: oaslananka/a2a-mesh)
"""
...
class GitHubClient:
    ...
    def get_latest_release(self) -> Dict:
        ...
        return self._request("GET", f"/repos/{self.repo}/releases/latest")

    def list_releases(self, count: int = 10) -> List[Dict]:
        """List GitHub releases."""
        result = self._request("GET", f"/repos/{self.repo}/releases?per_page={count}")
        return result if isinstance(result, list) else []

    def compare_commits(self, base: str, head: str) -> Dict:
        """Fetch the diff between two commits for sync checks."""
...
def cmd_github_release(args: argparse.Namespace) -> Dict:
    """Create a GitHub release from the Azure DevOps release pipeline."""
    pat = args.token or os.environ.get("GITHUB_PAT", "")
    repo = args.repo or os.environ.get("GITHUB_REPO", "")

    if not pat or not repo:
        raise RuntimeError("GITHUB_PAT and GITHUB_REPO are required.")

    github = GitHubClient(pat, repo)

    # Read changelog content
    body = args.notes or ""
    if not body and args.changelog and os.path.exists(args.changelog):
        with open(args.changelog, encoding="utf-8") as f:
            content = f.read()
        # Use the first section that starts with ## as the latest changelog entry
        sections = content.split("\n## ")
        if len(sections) > 1:
            body = "## " + sections[1]
        else:
            body = content[:2000]  # Max 2000 chars

    tag = args.tag
    name = f"Release {tag}"
    prerelease = any(s in tag for s in ("alpha", "beta", "rc", "dev"))

    print(f"\n{Color.BOLD}🏷️  Creating GitHub Release{Color.RESET}")
    print(f"   Repo:       {repo}")
    print(f"   Tag:        {tag}")
    print(f"   Pre-release: {'yes' if prerelease else 'no'}")

    release = github.create_release(tag, name, body, draft=False, prerelease=prerelease)
    url = release.get("html_url", "?")
    print(f"\n  {Color.GREEN}✓ Release created: {url}{Color.RESET}")
    return {"release_url": url, "tag": tag}


def cmd_sync_check(args: argparse.Namespace) -> Dict:
    """Check Azure DevOps ↔ GitHub mirror sync status."""
    pat = args.github_token or os.environ.get("GITHUB_PAT", "")
    repo = args.github_repo or os.environ.get("GITHUB_REPO", "")

    if not pat or not repo:
        print(
            f"  {Color.YELLOW}⚠ GitHub credentials not set. Skipping sync check.{Color.RESET}"
        )
        return {"status": "skipped"}

    print(f"\n{Color.BOLD}🔄 GitHub Mirror Sync Check{Color.RESET}")
    github = GitHubClient(pat, repo)

    try:
        repo_info = github.get_repo_info()
        releases = github.list_releases(5)

        print(f"  Repo:     {Color.CYAN}{repo}{Color.RESET}")
        print(f"  Default:  {repo_info.get('default_branch', '?')}")
        ...
        if releases:
            latest = releases[0]
            print(
                f"\n  Latest Release: {Color.GREEN}{latest.get('tag_name')}{Color.RESET}"
            )
            print(f"  Published:      {fmt_datetime(latest.get('published_at'))}")
        ...
        print(f"  {Color.RED}✗ GitHub sync check failed: {e}{Color.RESET}")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="azuredevops.py",
        description=(
            "Azure DevOps + GitHub integration utility.\n"
...
Examples:
  ...
  python3 scripts/azuredevops.py github-release --tag v1.2.0 --changelog CHANGELOG.md
  python3 scripts/azuredevops.py sync-check
...
Environment Variables:
  ...
  GITHUB_PAT, GITHUB_REPO
    ...
    # github-release
    p_release = subparsers.add_parser("github-release", help="Create a GitHub release")
    p_release.add_argument("--repo", help="GitHub repo (owner/repo)")
    p_release.add_argument("--token", help="GitHub PAT")
    p_release.add_argument("--tag", required=True, help="Release tag (for example: v1.2.0)")
    p_release.add_argument("--notes", help="Optional release notes text")
    p_release.add_argument(
        "--changelog", default="CHANGELOG.md", help="Path to CHANGELOG.md"
    )

    # sync-check
    p_sync = subparsers.add_parser("sync-check", help="Check GitHub mirror sync status")
    p_sync.add_argument(
        "--github-token", help="GitHub PAT (can also be read from GITHUB_PAT)"
    )
    p_sync.add_argument(
        "--github-repo", help="GitHub repo (can also be read from GITHUB_REPO)"
    )
...
def main() -> int:
    ...
    # GitHub-only commands (do not require Azure DevOps credentials)
    if args.command == "github-release":
        try:
            result = cmd_github_release(args)
            if args.json:
