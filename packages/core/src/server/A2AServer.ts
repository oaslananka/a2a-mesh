...
export abstract class A2AServer {
  ...
  private setupRoutes() {
    ...
    const handleStreamRequest = (req: Request, res: Response) => {
  ...
  private async handleMessageRequest(
    ...
    const task = params.taskId
      ? this.taskManager.getTask(params.taskId)
      : this.taskManager.createTask(params.sessionId, params.contextId ?? params.message.contextId);
