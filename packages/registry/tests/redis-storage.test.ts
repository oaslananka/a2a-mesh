...
describe('RedisStorage', () => {
  it('persists and retrieves JSON records through a redis-like client', async () => {
    ...
    const storage = new RedisStorage({
      ...
      async scan(cursor, matchOption, pattern, _countOption, _count) {
        const prefix = pattern.replace('*', '');
        const allKeys = Array.from(values.keys()).filter((key) => key.startsWith(prefix));
        // Simple mock of scan returning everything at once
        return ['0', allKeys];
      },
      async keys(pattern) {
        const prefix = pattern.replace('*', '');
