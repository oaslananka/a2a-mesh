...
describe('Registry Integration', () => {
  ...
  it('registers, searches, lists and manages agent records', async () => {
    ...
    await client.register('http://agent-1', {
      protocolVersion: '1.0',
      name: 'Agent 1',
      description: 'Desc',
      url: 'http://agent-1',
    ...
    await client.register('http://agent-2', {
      protocolVersion: '1.0',
      name: 'Agent 2',
      description: 'Desc',
      url: 'http://agent-2',
    ...
    expect(searches[0]?.url).toBe('http://agent-1');
  ...
  it('returns validation and not-found responses and streams registry updates', async () => {
    ...
    expect((await fetch(`${baseUrl}/agents/missing/heartbeat`, { method: 'POST' })).status).toBe(404);
    expect((await fetch(`${baseUrl}/agents/missing`, { method: 'DELETE' })).status).toBe(404);

    await client.register('http://agent-3', {
      protocolVersion: '1.0',
      name: 'Agent 3',
      description: 'Desc',
      url: 'http://agent-3',
    ...
    expect(firstEvent.value).toEqual(
      expect.objectContaining({
        type: 'registered',
        agent: expect.objectContaining({ url: 'http://agent-3' }),
      }),
    );
  ...
  it('marks agents healthy and unhealthy during scheduled health checks', async () => {
    ...
    vi.spyOn(globalThis, 'fetch').mockImplementation(async (input) => {
      ...
      if (url.startsWith('http://healthy-agent')) {
        return new Response(null, { status: 200 });
      }
      throw new Error('offline');
    });

    const server = new RegistryServer({ storage });
    (server as unknown as { startHealthChecks: () => void }).startHealthChecks();
